...
public class PokemonTab extends JPanel {
	...
	public PokemonTab(PokemonGo go) {
		...
		SwingWorker<Void, Void> transferSelectedTask = new SwingWorker<Void, Void>(){
			protected Void doInBackground() throws Exception {try { transferSelected(); } catch (Exception e) { e.printStackTrace(); } return null;}
		};
		transferSelected.addActionListener(l->transferSelectedTask.execute());
		topPanel.add(evolveSelected, gbc);
		SwingWorker<Void, Void> evolveSelectedTask = new SwingWorker<Void, Void>() {
			protected Void doInBackground() throws Exception {evolveSelected();return null;}
		};
		evolveSelected.addActionListener(l -> evolveSelectedTask.execute());
		topPanel.add(powerUpSelected, gbc);
		SwingWorker<Void, Void> powerUpSelectedTask = new SwingWorker<Void, Void>() {
			protected Void doInBackground() throws Exception {powerUpSelected();return null;}
		};
		powerUpSelected.addActionListener(l -> powerUpSelectedTask.execute());
